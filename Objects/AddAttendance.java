...
public class AddAttendance {
    ...
    public ResultSet dbSearch(String classes) throws SQLException {
        String str1 = "SELECT * from student where class = '"+classes+"'";
    ...
    public String[] classEt() throws SQLException {
        String str1 = "SELECT className from class";
        Statement stm = con.createStatement();
        ResultSet rst = stm.executeQuery(str1);
        String[] rt = new String[25];
        int i=0;
        while(rst.next()) {
            rt[i] = rst.getString("className");
            i++;
        }
        return rt;
    }
    ...
    public void tblupdt(String classes) {
        ...
        try {
            ...
            for(int i=0; res.next(); i++) {
                ...
                model.setValueAt(res.getInt("classId"), i, 0);
                model.setValueAt(res.getString("className"), i, 1);
    ...
    public void addItem(int id, String status, String date, String classes) throws SQLException {
        String adding = "INSERT INTO attendance values("+id+", '"+date+"', '"+status+"', '"+classes+"')";
        Statement stm = con.createStatement();
        stm.executeUpdate(adding);
    }

    public boolean check(String classes, String dt) throws SQLException {
        String str1 = "select * from attendance where class = '"+classes+"' AND dt = '"+dt+"'";
        Statement stm = con.createStatement();
        ResultSet rst = stm.executeQuery(str1);
        if(rst.next())
            return true;
