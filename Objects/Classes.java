...
public class Classes {
    ...
    public int getid() throws SQLException {
        ...
        ResultSet rst = stm.executeQuery("SELECT MAX(classId) from class");
        if(rst.next()) {
            return rst.getInt("MAX(classId)")+1;
        }
        else {
            return 1;
        }
    ...
    public void deleter(int id) throws SQLException {
        String del = "DELETE FROM class WHERE classId = "+id;
        Statement stm = con.createStatement();
        stm.executeUpdate(del);
    }
    public void editor(int id, String name) throws SQLException {
        String update = "UPDATE class SET className = '"+name+"'WHERE classId = "+id;
        Statement stm = con.createStatement();
